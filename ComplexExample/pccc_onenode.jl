...
println("Starting Common CCC SOS Simulation for One-Node Graph...")
...
h_X0(v)  = [3.0 - (v[2] - v[1]), (v[2] - v[1]) - 2.5]
h_Xvf(v) = [2.5 - (v[2] - v[1]), (v[2] - v[1]) - 1.0]

# Graph definition (Single Node)
nodes = [1]
# Edges: (p, sigma, q) -> transitions (v1, 1, v1) and (v1, 2, v1)
edges = [(1, 1, 1), (1, 2, 1)]

file = open("platoon_ccc_common_results.txt", "w")
...
for deg in 4:6
    ...
    # Dictionary for certificates C_{p,q} (Only C_{1,1} exists here)
    C = Dict()
    C_vars = [x; y]
    ...
    if status == MOI.OPTIMAL || status == MOI.ALMOST_OPTIMAL || status == MOI.SLOW_PROGRESS
        ...
        for p in nodes, q in nodes
            ...
            rounded_poly = sum((c * m for (c, m) in zip(val_coef, mono) if abs(c) > 0.0), init=0.0)
